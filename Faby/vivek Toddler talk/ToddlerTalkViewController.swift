...
class ToddlerTalkViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UISearchBarDelegate {
    ...
    private func loadCachedTopics() {
        ...
        // Always fetch fresh data from Supabase after checking cache
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
            self?.fetchTopicsFromSupabase()
        }
    }
    ...
    private func updateImageLoadingProgress() {
        ...
        if totalImagesCount > 0 {
            percentage = Int((Float(loadedImagesCount) / Float(totalImagesCount)) * 100)
        } else {
            percentage = 0
        }
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.loadingLabel.text = "Gathering info... \(percentage)%"
        }
