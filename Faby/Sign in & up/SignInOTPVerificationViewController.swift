...
// Protocol for handling sign-in OTP verification completion
protocol SignInOTPVerificationViewControllerDelegate: AnyObject {
    func didVerifySignInOTP(email: String)
    func didCancelSignInOTP()
}

class SignInOTPVerificationViewController: UIViewController {
    ...
    weak var delegate: SignInOTPVerificationViewControllerDelegate?
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // Save session in UserSessionManager
                    if let session = response.session {
                        UserSessionManager.shared.supabaseSession = session.accessToken
                        UserSessionManager.shared.userEmail = userEmail
                        UserSessionManager.shared.completeSignup()
                    }
                    
                    // Notify delegate
                    delegate?.didVerifySignInOTP(email: userEmail)
                }
            } catch {
                await MainActor.run {
                    activityIndicator.stopAnimating()
...
extension SignInOTPVerificationViewController: UITextFieldDelegate {
