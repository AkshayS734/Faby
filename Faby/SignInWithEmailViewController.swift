...
class SignInWithEmailViewController: UIViewController,UITextFieldDelegate {
    ...
    private let skipLoginButton: UIButton = {
        ...
        button.setTitle("Skip Login (Quick Access)", for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        button.setTitleColor(UIColor(hex: "#06D6A0"), for: .normal)
    ...
    override func viewDidLoad() {
        ...
        skipLoginButton.addTarget(self, action: #selector(skipLoginPressed), for: .touchUpInside)
        
        // Add constraints for skip login button
        NSLayoutConstraint.activate([
            skipLoginButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            skipLoginButton.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -20)
        ])
    }
    deinit {
    ...
    @objc private func skipLoginPressed() {
        navigateToHome()
    }
}
